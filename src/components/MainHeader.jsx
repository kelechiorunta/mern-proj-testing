  ...
  return (
    ...
    bg-[#1e4548] py-3 px-8 z-20 sticky top-0'>
        ...
        xsm:max-sm:hidden`}>
            ...
            <Link 
            onClick={localStorage.getItem('currentUser') && handlelogout}
            className={` ${loading && ' transition-all duration-200 ease-in-out animate-bounce bg-transparent font-semibold '}`}
            to={!localStorage.getItem('currentUser') && '/auth/login-user'}>
            {
